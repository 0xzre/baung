...
const LoadMoreButton = ({ onClick, isLoading }) => (
  <button
    className="bg-primary hover:bg-green-700 text-sm text-white font-semibold rounded-md w-full p-2 my-3"
    onClick={onClick}
    disabled={isLoading}
  >
    ...
    <>
      ...
      {posts.length === 0 && (
        <div className="text-center text-gray-700 flex justify-center items-center flex-col">
          ...
          <img className="h-[280px] w-auto sm:h-[470px]" loading="lazy" src={Home} alt="no post" />
        </div>
      )}
    </>
  );
